...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.task_data = {}
        self.task_path = TASKS_DIR / "JieJieKa.json"
        self.windows = []
        self.mumu_devices = []
    ...
    def show_splash(self):
        ...
        splash.configure(bg="#0f1420")
        width, height = 360, 140
        x = max(0, (self.winfo_screenwidth() - width) // 2)
        y = max(0, (self.winfo_screenheight() - height) // 2)
        splash.geometry(f"{width}x{height}+{x}+{y}")
        frame = tk.Frame(splash, bg="#0f1420", bd=1, relief="solid")
        frame.pack(fill="both", expand=True, padx=1, pady=1)
        tk.Label(frame, text="脚本助手", font=("Microsoft YaHei UI", 18, "bold"), fg="#e7ecf6", bg="#0f1420").pack(pady=(28, 8))
        self.splash_status = tk.StringVar(value="正在初始化界面...")
        tk.Label(frame, textvariable=self.splash_status, font=("Microsoft YaHei UI", 10), fg="#8f9bb3", bg="#0f1420").pack()
        self.splash = splash
        splash.update_idletasks()

    def set_splash_status(self, text):
        if self.splash is not None and self.splash.winfo_exists():
            self.splash_status.set(text)
            self.splash.update_idletasks()

    def finish_startup(self):
        self.set_splash_status("正在加载配置...")
        self.load_config()
        self.set_splash_status("正在加载任务...")
        self.refresh_tasks()
        self.set_splash_status("正在扫描窗口...")
        self.refresh_windows()
        self.set_splash_status("正在扫描设备...")
        self.refresh_adb()
        self.set_splash_status("正在完成布局...")
    ...
    def create_instance(self, name, config_path=None, make_active=False):
        ...
        if config_path is None:
            ...
            config_path = CONFIG_DIR / f"config_{safe}.json"
        else:
            config_path = Path(config_path)

        if not config_path.exists():
            base = load_json(getattr(self, "config_path", CONFIG_DIR / "config.json"), {})
    ...
    def add_instance(self):
        ...
        ascii_path = CONFIG_DIR / f"config_instance_{index}.json"
    ...
    def switch_instance(self, name):
        ...
        self.instance_var.set(name)
        if hasattr(self, "device_candidate_var"):
            self.device_candidate_var.set(item.get("device_candidate", ""))
        self.load_config()
        image = item.get("preview_image")
        if image is not None:
            self.preview_image = image
            self.preview_status = ""
            self.preview_canvas.itemconfigure(self.preview_image_id, image=image, state="normal")
        ...
        self.sync_device_toggle_button()
        self.sync_preview_toggle_button()

    def active_instance(self):
        return self.instances[self.active_instance_name]

    def remember_device_candidate(self):
        if not self.instances or not hasattr(self, "device_candidate_var"):
            return
        self.active_instance()["device_candidate"] = self.device_candidate_var.get()

    def sync_device_toggle_button(self):
        if not hasattr(self, "device_toggle_btn") or not self.instances:
            return
        connected = self.active_instance().get("device") is not None
        self.device_toggle_btn.configure(
            text="断开设备" if connected else "连接设备",
            fg_color="#202635" if connected else "#5636e8",
        )

    def sync_preview_toggle_button(self):
        if not hasattr(self, "preview_toggle_btn") or not self.instances:
            return
        running = bool(self.active_instance().get("preview_running"))
        self.preview_toggle_btn.configure(
            text="停止预览" if running else "开启预览",
            fg_color="#202635" if running else "#5636e8",
        )

    def toggle_device_connection(self):
        if self.active_instance().get("device"):
            self.disconnect_selected_device()
        else:
            self.connect_selected_device()

    def toggle_preview(self):
        if self.active_instance().get("preview_running"):
            self.stop_preview()
        else:
            self.start_preview()

    def on_preview_canvas_resize(self, _event=None):
        width = max(1, self.preview_canvas.winfo_width() // 2)
        height = max(1, self.preview_canvas.winfo_height() // 2)
        self.preview_canvas.coords(self.preview_image_id, width, height)
        self.preview_canvas.coords(self.preview_text_id, width, height)
    ...
    def show_preview_status(self, text):
        ...
        self.preview_canvas.itemconfigure(self.preview_text_id, text=text, state="normal")
    ...
    def build_status_cards(self, parent):
        ...
        self.device_candidate_var.trace_add("write", lambda *_: self.remember_device_candidate())
        ...
        self.device_toggle_btn = ctk.CTkButton(self.device_card, text="连接设备", width=90, command=self.toggle_device_connection)
        self.device_toggle_btn.grid(row=3, column=1, columnspan=2, sticky="ew", padx=(5, 16), pady=(0, 14))
    ...
    def build_right_panel(self, parent):
        ...
        self.preview.bind("<Enter>", lambda _e: "break")
        self.preview.bind("<Motion>", lambda _e: "break")
        self.preview_canvas = tk.Canvas(self.preview, bg="#0b0f18", highlightthickness=0, bd=0, cursor="arrow")
        self.preview_canvas.place(relx=0, rely=0, relwidth=1, relheight=1)
        self.preview_image_id = self.preview_canvas.create_image(0, 0, anchor="center", state="hidden")
        self.preview_text_id = self.preview_canvas.create_text(
            0, 0, text="预览区域\n后续可接入截图流", fill="#667085", justify="center"
        )
        self.preview_canvas.bind("<Configure>", self.on_preview_canvas_resize)
        for sequence in ("<Enter>", "<Leave>", "<Motion>", "<Button-1>", "<ButtonRelease-1>", "<MouseWheel>"):
            self.preview_canvas.bind(sequence, lambda _e: "break")
        ...
        self.preview_toggle_btn = ctk.CTkButton(controls, text="开启预览", fg_color="#202635", command=self.toggle_preview)
        self.preview_toggle_btn.grid(row=0, column=1, sticky="ew", padx=(6, 0))
    ...
    def build_log(self, parent):
        ...
        top.grid(row=3, column=0, sticky="ew", padx=16, pady=(0, 8))
        ...
        self.log_text.grid(row=4, column=0, sticky="nsew", padx=16, pady=(0, 16))
    ...
    def update_device_candidate_menu(self):
        ...
        selected = self.active_instance().get("device_candidate", "") if self.instances else ""
        if selected in values:
            self.device_candidate_var.set(selected)
        elif values:
            self.device_candidate_var.set(values[0])
            if self.instances:
                self.active_instance()["device_candidate"] = values[0]
    ...
    def selected_candidate_device(self):
        ...
        if self.instances:
            self.active_instance()["device_candidate"] = selected
    ...
    def refresh_tasks(self, select=None):
        ...
        if files:
            ...
            self.load_task_file(TASKS_DIR / default)

    def on_task_file_selected(self, name):
        self.load_task_file(TASKS_DIR / name)
    ...
    def next_task_name(self):
        index = 1
        while (TASKS_DIR / f"CustomTask{index}.json").exists():
            index += 1
    ...
    def open_task_file(self):
        path = filedialog.askopenfilename(initialdir=TASKS_DIR, filetypes=[("JSON文件", "*.json")])
        if path:
    ...
    def find_preview_hwnd_for_config(self, cfg):
        title = str(cfg.get("windowName") or "")
        target_name = str(cfg.get("targetWindowName") or "")
        if not title:
            return 0
        ...
        def resolve():
            for row in self.windows:
                if row["title"] != title:
                    continue
                if target_name:
                    child = find_descendant_by_text_or_class(row["hwnd"], target_name)
                    if child:
                        return child
                return row["hwnd"]
            return 0
    ...
    def capture_preview_for_instance(self, item, max_width, max_height):
        ...
        backend = str(cfg.get("captureBackend", "wgc"))
        if backend == "adb":
            return adb_preview_png(cfg.get("adbPath", "adb"), cfg.get("deviceSerial", ""))

        hwnd = self.find_preview_hwnd_for_config(cfg)
        if not hwnd:
            return None
        return capture_window_ppm(hwnd, max_width=max_width)

    def start_preview(self):
        self.save_config()
        item = self.active_instance()
        instance_name = self.active_instance_name
        if self.capture_backend.get() != "adb" and not self.window_name.get():
            self.show_preview_status("设备未连接")
            self.log(f"{instance_name} 预览未能启动: 未连接设备")
            self.sync_preview_toggle_button()
            return
        if item["preview_running"]:
            self.log(f"{instance_name} 预览已在运行")
            self.sync_preview_toggle_button()
            return
        if learnopencv_py is not None:
            learnopencv_py.start_capture(str(item["config_path"]), 30)
        item["preview_running"] = True
        self.log(f"{instance_name} 预览已启动，帧率 {self.preview_fps.get()} FPS")
        self.sync_preview_toggle_button()
        self.after(100, lambda name=instance_name: self.preview_tick(name))

    def stop_preview(self):
        item = self.active_instance()
        instance_name = self.active_instance_name
        if not item["preview_running"]:
            self.log(f"{instance_name} 预览未在运行")
            self.sync_preview_toggle_button()
            return
        ...
        self.log(f"{instance_name} 预览已停止")
        self.sync_preview_toggle_button()
        ...
        def worker():
            try:
                if item["capture_lock"].acquire(timeout=2.0):
                    try:
                        data = self.capture_preview_for_instance(item, max_width, max_height)
                    finally:
    ...
    def apply_preview_frame(self, instance_name, data, error):
        ...
        try:
            ...
            if not data:
                ...
                return
            image = tk.PhotoImage(master=self.preview_canvas, data=data)
            item["preview_image"] = image
            if instance_name == self.active_instance_name:
                self.preview_image = image
                self.preview_status = ""
                self.on_preview_canvas_resize()
                self.preview_canvas.itemconfigure(self.preview_image_id, image=image, state="normal")
                self.preview_canvas.itemconfigure(self.preview_text_id, state="hidden")
        finally:
